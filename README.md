...
| Gold | `llp-gold` | FusionEngine / ReviewSvc / GoldRepublisher | L2 write=DataEngineer(fusion), L3 write=Reviewer(검수 완료), republish=Admin, read=Viewer | **버전 보존**(L2/L3 재발행 시 이전 버전 유지, rollback 가능) |

격리 시행 방법:

- 버킷 분리 + IAM/버킷 정책으로 계층 간 교차 쓰기를 원천 차단한다. Bronze는 ingest
  역할만, Gold는 승격 경로별 권한(L2=DataEngineer, L3=Reviewer, republish=Admin)만 write.
...
- feature↔label 조인은 현재 MVP에서 SQLAlchemy/SQLite로 수행하며, 결과만 Gold에 쓴다.
  레이크 조인 엔진은 DuckDB(의존성만 선언, [설계]) → Trino/Spark(V1)로 전환 예정이다.
